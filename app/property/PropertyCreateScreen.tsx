  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isEditMode 
        ? `${process.env.EXPO_PUBLIC_API_URL || 'https://trusted-frank-mudfish.ngrok-free.app'}/api/properties/${propertyId}`
        : `${process.env.EXPO_PUBLIC_API_URL || 'https://trusted-frank-mudfish.ngrok-free.app'}/api/properties`;
      ...
      if (!response.ok) throw new Error(data.message || `Failed to ${isEditMode ? 'update' : 'create'} property`);
      setSuccess(true);
      
      if (isEditMode) {
        navigation.navigate('PropertyDetails', { id: data._id || data.id || propertyId });
      } else {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: currentTheme.colors.background }}>
      <ScrollView contentContainerStyle={{ flexGrow: 1, padding: currentTheme.spacing.md }} showsVerticalScrollIndicator={false}>
        <VStack space="lg" style={{ width: '100%' }}>
          ...
          <Button action="primary" onPress={handleSubmit} disabled={saving} style={{ marginTop: currentTheme.spacing.lg }}>
            <ButtonText>{saving ? (isEditMode ? 'Saving...' : 'Creating...') : (isEditMode ? 'Save Changes' : 'Create Property')}</ButtonText>
          </Button>
        </VStack>
      </ScrollView>
    </SafeAreaView>
